...
func StartLocalServer(l *config.LocalServer, config *config.CommonConfig) error {
	...
	switch l.Type {
	case "p2ps", "p2pt":
		// 这两种模式原本调用 ehang.io/nps/server/proxy 在本地起 socks5 / tcp
		// 转发服务，而该包会把整个服务端代理栈拖进客户端二进制
		// （实测：客户端因此从 7.9MB 涨到 15.4MB，依赖包 202 → 307，
		// beego / gopsutil / kcp-go / nps-web 等全被链进来）。
		//
		// Sulink Lan 的客户端只以 bridge 模式连 NPS（NewRPClient + nil 配置），
		// cnf.LocalServer 始终为空，这个函数根本不会被调用；
		// 而 p2ps/p2pt 是 NPS 自身的本地转发特性，与本项目无关。
		// 因此改为明确报错而非静默失效——真有人配了这两种模式时能立刻看出来，
		// 而不是对着一个不工作的端口干等。
		return fmt.Errorf("本地转发模式 %q 不受支持（该功能未编入本客户端）", l.Type)
	case "p2p", "secret":
