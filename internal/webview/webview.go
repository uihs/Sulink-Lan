...
// Package webview 是 github.com/jchv/go-webview2 顶层的最小本地分支（fork）。
//
// 为什么 fork：上游在注册窗口类时没有设置 HbrBackground（背景画刷为 NULL），
// 客户区从不被擦除。窗口在 New() 内部就 ShowWindow 上屏，而 WebView2 控制器的
// 创建要等 New() 内部的消息泵跑完才完成——这几十到几百毫秒里用户看到的是
// 未初始化的黑色表面，表现为「启动闪黑屏」。上游把「建窗口-显示-初始化」
// 绑死在 New() 里且没有外部窗口入口（WebViewOptions.Window 在该版本是死参数），
// 从外部无法补救，只能改这一处。
//
// 与上游的差异刻意保持最小（仅窗口类背景画刷一处），便于日后对齐新版：
//   - internal/webview/w32  = 上游 internal/w32 原样拷贝 + CreateSolidBrush
//   - pkg/edge 仍走正式依赖，不在本 fork 内
//
// 以下是上游原文件的完整实现。
package webview
...
import (
	...
	"sulink-lan/internal/webview/w32"
...
func NewWithOptions(options WebViewOptions) WebView {
	...
	// 通过环境变量预设 WebView2 默认背景色，消除启动闪黑屏/白屏。
	// API 调用 PutDefaultBackgroundColor 在生效前仍有已知闪烁问题；
	// 环境变量在环境创建时就生效，可避免该间隙（Microsoft 官方建议）。
	// 格式：8 位十六进制，前 2 位 alpha，后 6 位 RGB。
	// #F7F9FA → A=FF, R=F7, G=F9, B=FA → FFF7F9FA。
	os.Setenv("WEBVIEW2_DEFAULT_BACKGROUND_COLOR", "FFF7F9FA")
...
func (w *webview) CreateWithOptions(opts WindowOptions) bool {
	...
	// [本地 fork 修改点] 给窗口类设置背景画刷，颜色与前端页面底色一致（#F7F9FA）。
	//
	// 上游此处 HbrBackground 为 NULL，含义是「客户区不要擦除」：窗口在下方
	// ShowWindow 时就已上屏，而 WebView2 控制器要等本函数末尾 Embed 的消息泵
	// 跑完才创建，这段时间客户区是未初始化的黑色表面 → 启动闪黑屏。
	// 设成与页面一致的浅色后，这个间隙露出来的是页面底色，肉眼无感。
	//
	// COLORREF 布局是 0x00BBGGRR：#F7F9FA → B=0xFA,G=0xF9,R=0xF7 → 0x00FAF9F7。
	// 画刷随窗口类常驻进程，一次创建不释放（进程退出由系统回收）。
	bgBrush, _, _ := w32.User32CreateSolidBrush.Call(0x00FAF9F7)
	wc := w32.WndClassExW{
